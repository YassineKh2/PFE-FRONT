...
export function ByRisk(Funds : Fund[]){
    return function (riskLevel: "Low" | "Moderate" | "High" | "Very-High"): Fund[] {
...
export function calculateLiquidityScore(fund: Fund, userLiquidityNeeds: number): number {
    let score = 0;
  ...
  export function computePreferenceScore(
    ...
    if (preferences.assetAllocation.includes("equity") && fund.category === "Equity") score += 20;
    if (preferences.assetAllocation.includes("debt") && fund.category === "Debt") score += 20;
    if (preferences.assetAllocation.includes("real-estate") && fund.category === "Real Estate") score += 20;
    if (preferences.assetAllocation.includes("commodities") && fund.category === "Commodities") score += 20;
    if (preferences.assetAllocation.includes("fixed-income") && fund.category === "Fixed Income") score += 20;
    if (preferences.assetAllocation.includes("alternative-investments") && fund.category === "Alternative Investments") score += 20;
    if (preferences.assetAllocation.includes("cash-equivalents") && fund.category === "Cash Equivalents") score += 20;
    if (preferences.assetAllocation.includes("index") && fund.category === "Index") score += 20;
  
    // --- Sector Preference Scoring ---
    if (preferences.sectorPreference.includes("technology") && fund.sector === "Technology") score += 10;
    if (preferences.sectorPreference.includes("healthcare") && fund.sector === "Healthcare") score += 10;
    if (preferences.sectorPreference.includes("finance") && fund.sector === "Finance") score += 10;
    if (preferences.sectorPreference.includes("energy") && fund.sector === "Energy") score += 10;
    if (preferences.sectorPreference.includes("consumer-goods") && fund.sector === "Consumer Goods") score += 10;
    if (preferences.sectorPreference.includes("utilities") && fund.sector === "Utilities") score += 10;
    if (preferences.sectorPreference.includes("real-estate") && fund.sector === "Real Estate") score += 10;
    if (preferences.sectorPreference.includes("industrial") && fund.sector === "Industrial") score += 10;
    if (preferences.sectorPreference.includes("telecommunications") && fund.sector === "Telecommunications") score += 10;
    if (preferences.sectorPreference.includes("materials") && fund.sector === "Materials") score += 10;
  
    // --- Asset Allocations to Avoid Scoring ---
    if (sectorToAvoid.includes("equity") && fund.category === "Equity") score -= 20;
    if (sectorToAvoid.includes("debt") && fund.category === "Debt") score -= 20;
    if (sectorToAvoid.includes("real-estate") && fund.category === "Real Estate") score -= 20;
    if (sectorToAvoid.includes("commodities") && fund.category === "Commodities") score -= 20;
    if (sectorToAvoid.includes("fixed-income") && fund.category === "Fixed Income") score -= 20;
    if (sectorToAvoid.includes("alternative-investments") && fund.category === "Alternative Investments") score -= 20;
    if (sectorToAvoid.includes("cash-equivalents") && fund.category === "Cash Equivalents") score -= 20;
    ...
    if (preferences.taxConsideration === "taxHighBracket" && taxEfficiencyScore >= 90) score += 10;
    if (preferences.taxConsideration === "taxEfficient" && taxEfficiencyScore >= 80) score += 10;
    if (preferences.taxConsideration === "taxAdvantaged" && taxEfficiencyScore >= 70) score += 10;
    if (preferences.taxConsideration === "taxHarvesting" && taxEfficiencyScore >= 60) score += 10;
  ...
  export function computeFinancialScore(
    ...
    if (goalTimeFrame < 5) { // short term (<5 months) – typically want safer funds
      if (fund.risk === "Low") score += 20;
      else if (fund.risk === "Moderate") score += 10;
      else score -= 5;
    } else { // long-term or moderate-term goals
    ...
    const savingsRatio =
      monthlyExpense > 0 ? netSavings / monthlyExpense : 0;
    if (savingsRatio >= 0.5) {
